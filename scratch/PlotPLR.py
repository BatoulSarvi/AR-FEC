...
if __name__ == '__main__':
	...
	PLR_list = [l.rstrip("\n").split(',') for l in lines]       # gop , actual plr,  predicted PLR

	gopArray = [0] * len(PLR_list)
	actualPLRArray = [0] * len(PLR_list)
	predPLRArray = [0] * len(PLR_list)
	
	for i in range (len(PLR_list)): 				 # gop , actual plr,  predicted PLR
	...
	# actualPLRArray = actualPLRArray[2:]
	# predPLRArray = predPLRArray[2:]

	font1 = {'family':'serif','color':'blue','size':12}
	font2 = {'family':'serif','color':'darkred','size':12}

	plt.plot(actualPLRArray, marker='*', linestyle='-', color='orange', ms = 5, mec = 'orange', mfc = 'orange', label='Actual PLR')
